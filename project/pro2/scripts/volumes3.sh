...
if [ "`echo -n $DEVICE_FS`" == "" ] ; then 
  ...
  lvcreate --name volume3 -l 100%FREE data

  # Create a file system on the volume
  mkfs.ext4 /dev/data/volume3
fi
...
echo '/dev/data/volume3 /data ext4 defaults 0 0' >> /etc/fstab

# Mount /data
mount /data

# Run server template of autogroup
yum update -y
yum install httpd -y
sed -i "s,/var/www/html,/data/www/html,g" /etc/httpd/conf/httpd.conf
systemctl start httpd
systemctl enable httpd
echo "This is server in AWS Region US-EAST-1" > /data/www/html/index.html
